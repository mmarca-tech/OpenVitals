...
class HeartRepositoryTest {
    ...
    @Test fun `instant range returns empty for an inverted or empty window`() = runTest {
        val start = Instant.parse("2026-07-11T10:00:00Z")
        val end = Instant.parse("2026-07-11T11:00:00Z")
        val hc = mockk<HealthConnectManager>()
        every { hc.availability() } returns HealthConnectAvailability.AVAILABLE
        coEvery { hc.grantedPermissions() } returns setOf(heartRatePermission)
        coEvery { hc.readRawHeartRateSamples(any(), any()) } returns listOf(
            HeartRateSample(start, 120L, "gadgetbridge"),
        )
        val repository = HeartRepositoryImpl(hc)

        assertEquals(emptyList<HeartRateSample>(), repository.loadHeartRateSamples(end, start))
        assertEquals(emptyList<HeartRateSample>(), repository.loadHeartRateSamples(start, start))
        coVerify(exactly = 0) { hc.readRawHeartRateSamples(any(), any()) }
    }

    @Test fun `instant range returns empty without the heart-rate permission`() = runTest {
        val start = Instant.parse("2026-07-11T10:00:00Z")
        val end = Instant.parse("2026-07-11T11:00:00Z")
        val hc = mockk<HealthConnectManager>()
        every { hc.availability() } returns HealthConnectAvailability.AVAILABLE
        ...
        assertEquals(
            ...
            HeartRepositoryImpl(hc).loadHeartRateSamples(start, end),
        )
        coVerify(exactly = 0) { hc.readRawHeartRateSamples(any(), any()) }
    }

...
class OpenVitalsConnectedFlowTest {
    ...
    fun manualEntryWidgetFlow_routesVisibleTilesAndEditControls() {
        ...
        composeRule.onNodeWithText(string(R.string.metric_weight)).performClick()
    ...
    fun metricDetailScaffoldFlow_coversPeriodNavigationAndTopBarActions() {
            ...
            OpenVitalsTheme {
                OpenVitalsAdaptiveScaffold(
                    ...
                    MetricDetailScaffold(
                        ...
                        onPreviousPeriod = { previousCount += 1; selectedDate = selectedDate.minusWeeks(1) },
                        onNextPeriod = { nextCount += 1; selectedDate = selectedDate.plusWeeks(1) },
                        onSelectDate = {},
                    ) { period ->
                        item {
        ...
        composeRule.waitForIdle()
        val afterPrevious = renderedPeriod()
        ...
        assertNotEquals("stepping back must move the period", startingPeriod, afterPrevious)
    ...
    /** The period the scaffold computed, read back off the rendered content. */
    private fun renderedPeriod(): String = composeRule
        .onNode(hasTextStartingWith("Period "))
        .fetchSemanticsNode()
    ...
    private fun hasTextStartingWith(prefix: String) =
        androidx.compose.ui.test.SemanticsMatcher("text starts with '$prefix'") { node ->
            val texts = node.config
                .getOrElse(androidx.compose.ui.semantics.SemanticsProperties.Text) { emptyList() }
            texts.any { annotated -> annotated.text.startsWith(prefix) }
        }

...
import tech.mmarca.openvitals.ui.components.LocalChartAggregationMode
...
internal fun Intent.openVitalsRoute(): String? =
...
internal fun migratedOpenVitalsRoute(route: String): String =
    if (route == LegacyDailyReadinessRoute) {
        Screen.BodyEnergyDetails.createRoute(LocalDate.now().toString())
    } else {
        route
    }

private const val LegacyDailyReadinessRoute = "daily_readiness"

internal fun isSupportedOpenVitalsRoute(route: String): Boolean {
...
private val RouteImportExtensions = setOf("gpx", "kml", "kmz", "fit", "tcx")

 ...
 *
 * The widening is unconditional. Asking for the exact window first and widening
 * only when that came back EMPTY catches a record that swallows the window
 * whole, and misses one that overlaps only its edge. A hike starting at 11:50
 * whose watch had been recording since 11:48 read back fine: the records from
 * 12:44 onwards begin inside the window, so the read was full, healthy-looking,
 * and short by the first fifty-three minutes. Nothing is empty, so nothing asked
 ...
 *
 * This is paid on every read rather than only on one that came back empty, which

...
import tech.mmarca.openvitals.domain.model.ActivityIntradayBucketDuration
...
internal class ActivityHealthReader(
            ...
            support.client().aggregateGroupByDuration(
                ...
                DailySteps(
                    ...
                    steps = if (includeSteps) {
                        day.totalLong { it[StepsRecord.COUNT_TOTAL] }
                    } else {
                        0L
                    },
                    distanceMeters = if (includeDistance) {
                        day.total { it[DistanceRecord.DISTANCE_TOTAL]?.inMeters }
                    } else {
                        0.0
                    },
                    wheelchairPushes = if (includeWheelchairPushes) {
                        day.totalLong { it[WheelchairPushesRecord.COUNT_TOTAL] }
                    } else {
                        null
                    },
                    floorsClimbed = if (includeFloors) {
                        day.total { it[FloorsClimbedRecord.FLOORS_CLIMBED_TOTAL] }.toInt()
                    } else {
                        null
                    },
                    activeCaloriesKcal = if (includeActiveCalories) {
                        day.total { it[ActiveCaloriesBurnedRecord.ACTIVE_CALORIES_TOTAL]?.inKilocalories }
                    } else {
                        null
                    },
                    elevationGainedMeters = if (includeElevation) {
                        day.total { it[ElevationGainedRecord.ELEVATION_GAINED_TOTAL]?.inMeters }
                    } else {
                        null
                    },
                )
    ...
    /**
     * The intraday series: running totals per five-minute aggregate bucket,
     * the Body Energy timeline's bucket. Today stops at now. A full day is
     * two requests. Health Connect prorates a record across the buckets it
     * spans, so the last point can differ from the day total by rounding.
     */
    suspend fun readActivityProgress(
        ...
        val end = if (date == LocalDate.now(zone)) Instant.now() else date.plusDays(1).atStartOfDay(zone).toInstant()
        ...
        return support.withLogging("readActivityProgress[$date][$start..$end]", emptyList()) {
            ...
            // At most MaxInsightAggregateBuckets per request: a full day is two.
            val buckets = aggregateWindows(start, end, ActivityIntradayBucketDuration)
                .flatMap { (windowStart, windowEnd) ->
                    client.aggregateGroupByDuration(
                        AggregateGroupByDurationRequest(
                            metrics = metrics,
                            timeRangeFilter = TimeRangeFilter.between(windowStart, windowEnd),
                            timeRangeSlicer = ActivityIntradayBucketDuration,
                        )
                    )
                }
            ...
            buckets.map { bucket ->
                ...
                if (includeFloors) {
                    cumulativeFloors += bucket.result[FloorsClimbedRecord.FLOORS_CLIMBED_TOTAL] ?: 0.0
                }
                if (includeElevation) {
    ...
    suspend fun writeActivityEntries(requests: List<ActivityWriteRequest>): List<String> = withContext(Dispatchers.IO) {
        ...
        requests.forEach { validateActivityWriteRequest(it) }

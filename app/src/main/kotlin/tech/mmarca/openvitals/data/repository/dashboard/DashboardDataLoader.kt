...
class DashboardDataLoader @Inject constructor(
    ...
    private suspend fun loadDashboardUncached(
        ...
        val date = query.date
        val sleepWindow = query.sleepWindow
        ...
        val activeCaffeine = readIfNeeded(
            wants(DashboardMetric.CAFFEINE) && date == LocalDate.now(),
            readNutritionPermission,
            "activeCaffeine",
        ) {
            ...
            if (entries.isEmpty()) {
                ...
                CaffeineInsightCalculator.activeCaffeineMg(
                    entries = entries,
                    at = Instant.now(),
                    preferences = preferencesRepository?.caffeinePreferences() ?: CaffeinePreferences(),
                    bodyProfile = preferencesRepository?.bodyProfile() ?: BodyProfile(),
                ).takeIf { it > 0.0 }
            }
        }
        ...
        val hrvSamples = readIfNeeded(wants(DashboardMetric.HRV), readHrvPermission, "HRV samples") {
            hc.readHrvSamples(nightStart, effectiveDayEnd)
        }
        ...
        val spO2 = readIfNeeded(wants(DashboardMetric.SPO2), readSpO2Permission, "SpO2") {
            hc.readLatestSpO2InWindow(nightStart, dayEnd)
        }
        ...
        DashboardData(
            ...
            caffeineGrams = dailyMacros
                ...
                ?.takeIf { it > 0.0 },
            activeCaffeineMg = activeCaffeine?.await(),
    ...
    private fun supportedMetrics(): Set<DashboardMetric> {
        ...
        return DashboardMetric.entries.filterTo(mutableSetOf()) { metric ->
            val permissions = permissionsForMetric(metric, showOpenVitalsCalculatedCalories = false)

...
class HydrationRepositoryImpl @Inject constructor(
    ...
    override fun recentHydrationAmountsMilliliters(): List<Double> =
        preferencesRepository?.recentHydrationAmountsMilliliters().orEmpty()

    override fun recordRecentHydrationAmountMilliliters(milliliters: Double) {
    ...
    override suspend fun deleteHydrationEntryByClientRecordId(clientRecordId: String) {
        val granted = grantedPermissionsIfAvailable()
        if (writeHydrationPermission !in granted) {
            Log.w(TAG, "Skipping deleteHydrationEntryByClientRecordId missingCount=1")
            throw SecurityException("Missing Health Connect hydration write permission.")
        }
        hc.deleteHydrationEntryByClientRecordId(clientRecordId)
    }
}

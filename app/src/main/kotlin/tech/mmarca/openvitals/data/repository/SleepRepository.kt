...
class SleepRepositoryImpl @Inject constructor(
    ...
    override suspend fun loadSleepPeriod(
        ...
        return coroutineScope {
            val current = async {
                hc.readSleepData(windows.current.start, windows.current.end, sleepWindow)
            }
            val previous = async {
                hc.readSleepData(windows.previous.start, windows.previous.end, sleepWindow)
            }
            val baseline = async {
                hc.readSleepData(windows.baseline.start, windows.baseline.end, sleepWindow)
            }

    ...
    LazyColumn(
        ...
        itemsIndexedWithKey(state.form.blocks) { index, block ->
            WorkoutPlanBlockCard(
                ...
                onStepDurationUnitChanged = { stepId, unit -> viewModel.updateStepDurationUnit(block.id, stepId, unit) },
                onStepSetsChanged = { stepId, text -> viewModel.updateStepSets(block.id, stepId, text) },
                onStepSetRestChanged = { stepId, text -> viewModel.updateStepSetRest(block.id, stepId, text) },
                onStepSetRestUnitChanged = { stepId, unit -> viewModel.updateStepSetRestUnit(block.id, stepId, unit) },
                onStepWeightChanged = { stepId, text -> viewModel.updateStepWeight(block.id, stepId, text) },

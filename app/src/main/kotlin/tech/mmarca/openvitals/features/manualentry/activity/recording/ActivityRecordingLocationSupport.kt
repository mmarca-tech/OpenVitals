...
internal fun ActivityRecordingState.withAcceptedLocation(
    ...
    var routeBreakIndexes = this.routeBreakIndexes
    var lastMovementAt = this.lastMovementAt ?: startTime ?: point.time
    var totalIdleMillis = this.totalIdleMillis
    if (lastPoint != null) {
        if (!point.time.isAfter(lastPoint.time)) {
            return withDroppedLocation(accuracyMeters, point.time, ActivityGpsStatus.FIX)
        }

        val elapsedMillis = Duration.between(lastPoint.time, point.time)
            .toMillis()
            .coerceAtLeast(0L)
        val legDistanceMeters = lastPoint.recordingDistanceMetersTo(point)
        if (
            legDistanceMeters < minimumSampleDistanceMeters(recordingPreferences) ||
        ...
        val startsNewRouteSegment = recordingPreferences.routeGapMeters
            ?.let { legDistanceMeters > it.toDouble() }
            ?: false
        if (startsNewRouteSegment) {
            routeBreakIndexes = routeBreakIndexes + points.size
        } else {
            if (isImplausibleJump(lastPoint, point, legDistanceMeters, elapsedMillis, accuracyMeters)) {
                return withDroppedLocation(accuracyMeters, point.time, ActivityGpsStatus.FIX)
            }
            distanceIncrement = legDistanceMeters
            elevationIncrement = lastPoint.elevationGainMetersTo(point)
            elevationLossIncrement = lastPoint.elevationLossMetersTo(point)
            currentSpeedMetersPerSecond = legDistanceMeters / (elapsedMillis / 1_000.0)
            if (autoIdleEnabled) {
                val idleStartedAt = lastMovementAt.plusMillis(autoIdleTimeoutMillis)
    ...
    return copy(
        ...
        latestUiPoint = point,
        distanceMeters = distanceMeters + distanceIncrement,

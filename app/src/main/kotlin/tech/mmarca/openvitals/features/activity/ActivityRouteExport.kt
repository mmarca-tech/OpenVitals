...
internal fun Context.shareActivityRoute(
    ...
    runCatching {
        ...
        startActivity(
            Intent.createChooser(
                shareIntent,
                getString(R.string.activity_route_share_chooser_title),
            )
        )
    }

private fun Context.createActivityRouteViewIntent(workout: ExerciseData): Intent {
    val exportFile = createActivityRouteExportFile(workout, ActivityRouteExportFormat.GPX)
    val uri = FileProvider.getUriForFile(
        this,
        "$packageName.fileprovider",
        exportFile,
...
private fun Context.createActivityRouteExportFile(
    workout: ExerciseData,
    format: ActivityRouteExportFormat,
): File =

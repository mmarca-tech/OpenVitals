...
import tech.mmarca.openvitals.features.readiness.DailyReadinessCard
...
fun BodyEnergyDetailsScreen(
    ...
    readinessViewModel: DailyReadinessViewModel,
            ...
            if (!state.calibration.setupCompleted) {
                item {
                    BodyEnergyCalibrationCard(
                        ...
                        onResetPersonalTuning = viewModel::resetPersonalTuning,
...
/** The battery: the score header, then the day's curve, under one accent. */
@Composable
internal fun BodyEnergyCard(
    display: BodyEnergyDisplayState,
    modifier: Modifier = Modifier,
) {
    val accentColor = bodyEnergyColor(display.timeline?.currentScore)
    OpenVitalsCard(
        ...
        Column(modifier = Modifier.fillMaxWidth()) {
            BodyEnergySummaryContent(timeline = display.timeline, accentColor = accentColor)
            HorizontalDivider(modifier = Modifier.padding(horizontal = 16.dp))
            BodyEnergyTimelineContent(display = display)
        }
    }
}

@Composable
private fun BodyEnergySummaryContent(
    ...
    Column(
        ...
        BodyEnergyStat(
            ...
            value = stringResource(confidenceLabelRes(timeline?.confidence ?: BodyEnergyConfidence.NO_DATA)),
            body = timeline?.let { localizedConfidenceReason(it) }.orEmpty(),
        )
    }
}

@Composable
private fun BodyEnergyTimelineContent(display: BodyEnergyDisplayState) {
    Column(
        modifier = Modifier.padding(16.dp),
        verticalArrangement = Arrangement.spacedBy(12.dp),
    ) {
        Text(
            text = stringResource(R.string.body_energy_timeline_day_title),
...
@Composable
internal fun BodyEnergyReasonsCard(
...
@Composable
internal fun BodyEnergyInputsCard(
    display: BodyEnergyDisplayState,
    modifier: Modifier = Modifier,
) {
    OpenVitalsCard(modifier = modifier.fillMaxWidth()) {
...
@Composable
internal fun BodyEnergyCalculationCard(
    modifier: Modifier = Modifier,
) {
    OpenVitalsCard(modifier = modifier.fillMaxWidth()) {
        Column(
...
private fun BodyEnergySourcesCard(
    ...
    OpenVitalsCard(modifier = modifier.fillMaxWidth()) {
        Column(
            ...
            Text(
                text = stringResource(R.string.cardio_load_references_title),
                style = MaterialTheme.typography.titleSmall,
                fontWeight = FontWeight.SemiBold,
            )
            ReferenceLinkButton(
    ...
    stringResource(
        when (influence) {
            ...
            BodyEnergyPrimaryInfluence.EVERYDAY_ACTIVITY ->
                R.string.body_energy_influence_everyday_activity
...
private fun reasonDetail(influence: BodyEnergyPrimaryInfluence): String =
    stringResource(
        when (influence) {
            ...
            BodyEnergyPrimaryInfluence.EVERYDAY_ACTIVITY ->
                R.string.body_energy_reason_everyday_activity_detail
...
private fun confidenceLabelRes(confidence: BodyEnergyConfidence): Int =
    when (confidence) {
        BodyEnergyConfidence.HIGH -> R.string.body_energy_confidence_high
        BodyEnergyConfidence.MEDIUM -> R.string.body_energy_confidence_medium
        BodyEnergyConfidence.LOW -> R.string.body_energy_confidence_low
...
private fun localizedConfidenceReason(timeline: BodyEnergyTimeline): String =
    when (timeline.confidenceReasonCode) {
        BodyEnergyReasonCode.STRONG_CALIBRATION ->
            stringResource(R.string.body_energy_reason_strong_calibration)
        BodyEnergyReasonCode.OBSERVED_OR_AGE_CALIBRATION ->
            stringResource(R.string.body_energy_reason_observed_or_age)
        BodyEnergyReasonCode.INCOMPLETE_CALIBRATION ->
            stringResource(R.string.body_energy_reason_incomplete_calibration)
        BodyEnergyReasonCode.SPARSE_BUCKETS ->
            stringResource(R.string.body_energy_reason_sparse_buckets)
        BodyEnergyReasonCode.NO_USABLE_DATA ->
            stringResource(R.string.body_energy_reason_no_usable_data)
        BodyEnergyReasonCode.NO_TIMELINE_WINDOW ->
            stringResource(R.string.body_energy_reason_no_timeline_window)
        BodyEnergyReasonCode.NEEDS_HEART_RATE_OR_SLEEP ->
            stringResource(R.string.body_energy_reason_needs_hr_or_sleep)

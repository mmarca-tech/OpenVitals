...
internal fun LazyListScope.bodyContent(
    ...
    val hasAnyBodyData = hasAnyBodyData(state, unitFormatter)
...
internal fun hasAnyBodyData(
    ...
    bodyMetricData(state, state.summary, unitFormatter).any { it.latest != null || it.values.isNotEmpty() } ||
...
internal fun LazyListScope.selectedDateBodyEntries(
    ...
    item {
        val entries = bodyReadingItems(
            ...
            onEditBodyMeasurement = onEditBodyMeasurement,
            onDeleteBodyMeasurement = onDeleteBodyMeasurement,
        ).onDate(selectedDate, zone).newestFirst()
        PaginatedEntryList(

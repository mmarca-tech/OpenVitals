    ...
    Column(modifier = Modifier.padding(16.dp)) {
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                Text(
                    text = unitFormatter.duration(sleepDurationMsFromStages(session.stages, session.durationMs)),
                ...
                Text(
                    text = "${
                        unitFormatter.duration(session.endTime.toEpochMilli() - session.startTime.toEpochMilli())
                    } ${stringResource(R.string.sleep_time_in_bed).lowercase()}",
                    style = MaterialTheme.typography.bodySmall,
        ...
        if (sleepSessionHasReliableStages(session)) {
            ...
            Spacer(Modifier.height(16.dp))
            Text(
                text = stringResource(R.string.message_partial_stages),
                style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
            )
        }

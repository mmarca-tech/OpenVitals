...
internal fun bodyMetricData(
    ...
    listOf(
        ...
        BodyMetricData(
            ...
            latest = summary.adjustedFfmi?.let { DisplayValue(unitFormatter.decimal(it, 1), "") },
            values = emptyList(),
            color = BodyFatColor,
            icon = Icons.Outlined.FitnessCenter,
            valueDisplayFormatter = { DisplayValue(unitFormatter.decimal(it, 1), "") },
...
/** The readings that fall on [date], indexed by the reading's local day. */
internal fun List<BodyReadingItem>.onDate(
    date: LocalDate,

...
/**
 * The overview's top cards always render; the key-metrics list is period
 * only. On the day view each card repeats what the day shows better.
 */
@Composable
...
private fun SleepOverviewTopCards(
    ...
    Column(
        ...
        Row(
            ...
            SleepOverviewTopCard(
                title = stringResource(R.string.sleep_time_in_bed),
                value = durationOrNoData(summary.timeInBedMs, unitFormatter),
                subtitle = sleepOverviewAverageSubtitle(selectedRange, period),
                icon = Icons.Outlined.Hotel,
                modifier = Modifier.weight(1f),
            )
            SleepOverviewTopCard(
                title = stringResource(R.string.sleep_stage_awake),
                value = if (summary.timeInBedMs > 0L) {
                    DisplayValue(unitFormatter.duration(summary.awakeDurationMs), "")
                } else {
                    DisplayValue("--", "")
                },
                subtitle = sleepOverviewAverageSubtitle(selectedRange, period),

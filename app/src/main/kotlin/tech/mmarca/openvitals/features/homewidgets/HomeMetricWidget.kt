...
internal suspend fun writeHomeWidgetSnapshot(
    ...
    updateAppWidgetState(
        ...
        preferences.putHomeWidgetSnapshot(metricId, snapshot)
    }
}
...
internal fun MutablePreferences.putHomeWidgetSnapshot(
    ...
    if (metricId == null) {
        remove(HomeMetricWidgetState.metricIdKey)
    } else {
        this[HomeMetricWidgetState.metricIdKey] = metricId
    }
    this[HomeMetricWidgetState.titleKey] = snapshot.title
    this[HomeMetricWidgetState.valueKey] = snapshot.value
    ...
    for (index in 0 until MaxHomeWidgetRows) {
        ...
        if (row == null) {
            remove(homeWidgetRowLabelKey(index))
            remove(homeWidgetRowValueKey(index))
            remove(homeWidgetRowSubtitleKey(index))
        } else {
            this[homeWidgetRowLabelKey(index)] = row.label
            this[homeWidgetRowValueKey(index)] = row.value
            this[homeWidgetRowSubtitleKey(index)] = row.subtitle
        }
    }
}
...
internal fun buildMetricWidgetSnapshot(
    ...
    title: String = context.getString(metricId.homeMetricTitleRes()),
    route: String = homeMetricWidgetRoute(metricId, data.date),
): HomeMetricWidgetSnapshot =
...
internal fun buildBodyEnergyMetricSnapshot(
    context: Context,
...
internal const val MaxHomeWidgetRows = 12

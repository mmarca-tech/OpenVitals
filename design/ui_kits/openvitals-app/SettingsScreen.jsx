...
function SettingsScreen({ nav }) {
  const items = [
    ...
    ['devices', 'Sync with another phone', 'Copy Health Connect records to a nearby phone over Bluetooth'],
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, padding: '8px 16px 0' }}>
        {items.map(([icon, title, sub], i) => (
          <SettingsListItem key={i} icon={icon} title={title} supportingText={sub}
            onClick={() => { if (title === 'Display') nav('display'); if (title === 'Recovery') nav('readiness'); if (title === 'Nutrition') nav('beverage'); if (title === 'Sync with another phone') nav('devicesync'); }} />
        ))}

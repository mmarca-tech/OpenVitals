...
- **The app is Flutter/Dart**, and this system sits in its repo. When working on production code the real theme is `lib/ui/theme/` — `app_colors.dart`, `app_typography.dart`, `app_theme.dart`, `chart_tokens.dart`, `design_tokens.dart`. Values there win over values here; scales and component contracts here win over bare numbers there.

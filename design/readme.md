...
**OpenVitals is a Flutter/Dart app.** It was ported from Kotlin/Compose, and this
system lives in the same repository as the code it describes — paths below are
repo-relative:
...
> system never heard about it. If a value here disagrees with `lib/ui/theme/`,
> **the Dart is right** and this file is stale.
...
- **Scales and contracts** — the spacing grid, the radius scale, component
  ...
  these as named tokens in `lib/ui/theme/design_tokens.dart`; screens that still
  use bare numbers are being migrated onto it.

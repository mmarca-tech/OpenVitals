 ...
 *
 * **Why this is native.** The Garmin stack is Dart, and until now everything
 * that opened a link ran in the app's own isolate, where a Riverpod-scoped
 * `isSyncing` check was enough. Notification forwarding breaks that: it runs in
 * a HEADLESS Flutter engine, and each engine gets its own `FlutterBluePlusPlugin`
 * instance with its own `BluetoothGatt` map and its own method-call mutex.
 * Neither knows the other exists, and no Dart mutex can span them. Only
 * something process-wide can, and in a Flutter app that means Kotlin statics.
 *
 * **Keyed by address, not global.** Two engines talking to DIFFERENT peripherals
 * is fine. A single global lock would starve notifications for the whole of a
 * three-hour ride with a heart-rate strap connected, which is the wrong answer
 * for a wrist device.
 *
 * **Leases expire.** An isolate can be killed at any moment — that is the whole
 * premise of the headless engine — and a lease that outlived its holder would
 * wedge the radio until reboot. That failure already exists in this codebase
 * once (a connect against a radio still in use that never returns: no error, no
 * log, just a spinner), and it is not worth repeating.
 */
object RadioLeases {
     ...
     * range, which is indefinitely — so without a way to ask for it back, tapping
     * Sync would block until the user walked away from their own watch. A waiter
     * makes the holder's next [renew] fail, and a holder that cannot renew gives
     * the radio up.
     */
     ...
     *
     * Android closes a `BluetoothGatt` asynchronously. Opening a new one from a
     * DIFFERENT plugin instance before the old one has finished tearing down is
     * exactly the collision this class exists to prevent, so the lease outlives
     * the release by a moment.
     */
     ...
     * it. Re-acquiring a lease already held by [owner] succeeds and extends it,
     * so a caller need not track whether it already has one.
     */
    @Synchronized
     ...
     *
     * Does not grant anything — it makes the current holder's next [renew] fail,
     * which is how an indefinitely-held lease is given up. The caller then
     * retries [acquire].
     */
    @Synchronized

 ...
 *     rejecting them here — before any Flutter engine exists — is the single
 *     biggest battery decision in the feature.
 *  3. Buffer the survivors and wake the Dart forwarder.
 *
 * It touches no Bluetooth and holds no protocol knowledge. Nothing is written to
 * disk: notification text lives in a bounded in-memory buffer until Dart drains
 * it, and the app has no INTERNET permission, so it cannot leave the phone by
 * any route other than the one Bluetooth link.
 */
class OpenVitalsNotificationListenerService : NotificationListenerService() {
    ...
    companion object {
         ...
         * the bound service — and the request arrives from a Flutter engine that
    ...
    override fun onListenerConnected() {
        ...
        Diagnostics.i(this, TAG, "notification listener connected")
    }

    override fun onListenerDisconnected() {
        connected = false
        if (instance === this) instance = null
        Diagnostics.i(this, TAG, "notification listener disconnected")
    ...
    private fun handle(sbn: StatusBarNotification?, removed: Boolean) {
        ...
        try {
            ...
            if (verdict != NotificationFilter.Verdict.KEEP) {
                ...
                Diagnostics.d(this, TAG, "dropped ${sbn.packageName}: $verdict")
                return
            }
    ...
    private fun actionsOf(notification: Notification): List<NotificationActionMsg> {
        ...
        return wristActions(notification).mapIndexedNotNull { index, action ->
            ...
            NotificationActionMsg(
                ...
                fireableFromBackground = !action.actionIntent!!.isActivity,
            )
        }
    }
    ...
    private fun describeActions(notification: Notification) {
        if (!Diagnostics.isEnabled(this)) return
        val wearable = NotificationCompat.WearableExtender(notification).actions

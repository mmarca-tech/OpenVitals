...
```bash
./gradlew publishOpenVitalsArtifactsToMavenLocal -PopenVitalsArtifactVersion=1.2.0
```

or:

```bash
OPENVITALS_ARTIFACT_VERSION=1.2.0 ./gradlew publishOpenVitalsArtifactsToMavenLocal
```
...
./gradlew :openvitals-core-period:publishMavenPublicationToOpenVitalsReleaseRepository \
  -PopenVitalsArtifactVersion=1.2.0 \
...
```kotlin
dependencies {
    implementation("tech.mmarca.openvitals:openvitals-core-period:1.2.0")
}
```

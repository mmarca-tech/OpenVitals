...
Artifact versions default to `1.2.1-SNAPSHOT`.

Override the version with either:

```bash
./gradlew publishOpenVitalsArtifactsToMavenLocal -PopenVitalsArtifactVersion=1.2.1
```

or:

```bash
OPENVITALS_ARTIFACT_VERSION=1.2.1 ./gradlew publishOpenVitalsArtifactsToMavenLocal
```
...
./gradlew :openvitals-core-period:publishMavenPublicationToOpenVitalsReleaseRepository \
  -PopenVitalsArtifactVersion=1.2.1 \
...
```kotlin
dependencies {
    implementation("tech.mmarca.openvitals:openvitals-core-period:1.2.1")
}
```

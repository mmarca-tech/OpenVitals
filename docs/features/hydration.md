...
The daily hydration goal is set with a stepper in Settings, Nutrition, in the app's unit system. It drives goal progress, streaks, and the point at which hydration reminders pause for the day.
...
A reminder carries one-tap add buttons for recently used container sizes, so a drink can be logged without opening the app. Logging a drink re-anchors the countdown to that drink. See [Reminders](reminders.md).

...
## Body Profile Weight And Height

Settings, Body profile holds a weight and a height alongside birth year and heart-rate values, and they are the same numbers the body metrics use.

- When Health Connect has a recorded measurement, the field shows that value and is labelled as measured.
- Changing the weight or the height also writes it to Health Connect as a real measurement, so the app does not hold a second private copy of a number it already has. Saving an unchanged card writes nothing, so opening the section repeatedly does not litter the body history.
- When nothing has been recorded, or the app cannot write body measurements, the typed value is kept as a local preference and used the same way.

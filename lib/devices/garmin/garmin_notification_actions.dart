...
/// nothing about Android, which is what lets the whole GNCS conversation be
/// tested with no platform at all.
library;
...
/// draw them in.
const int maxGarminCustomActions = 5;
...
List<GarminNotificationAction> garminActionsFor(NotificationMsg message) {
  final actions = <GarminNotificationAction>[];

  if (message.dismissable) {
    actions.add(const GarminNotificationAction(
      kind: GarminNotificationActionKind.dismiss,
      // Never shown: the watch draws dismiss as an icon in a fixed position and
      // ignores the label. Named anyway so a log line reads sensibly.
      label: 'Dismiss',
      androidIndex: -1,
    ));
  }

  // At most ONE reply. The watch has a single reply control, so a second would
  // silently overwrite the first — better to offer the app's own first reply and
  // let the rest fall through to custom slots.
  var replyTaken = false;
  var customSlot = 0;

  for (final action in message.actions) {
    if (!action.fireableFromBackground) {
      // An action that opens the posting app's own screen. Android blocks a
      // background activity launch, so invoking it from the wrist does nothing
      // at all — no error, no effect. Offering it would put back exactly the
      // dead button this feature was built to remove. A stock SMS app's "Reply"
      // is one of these: it prefills a compose screen rather than sending.
      continue;
    }
    if (action.isReply && !replyTaken) {
      replyTaken = true;
      actions.add(GarminNotificationAction(
    ...
    if (customSlot >= maxGarminCustomActions) {
      // Dropped rather than crammed in: the slots ARE the wire format, and
      // reusing one would make two buttons invoke the same thing.
      continue;
    }
    actions.add(GarminNotificationAction(
      ...
      label: action.title,
      androidIndex: action.index,
      // A second reply loses its text box and becomes a plain button. It still
      // fires, which is better than not offering it — most apps treat an empty
      // reply as a no-op rather than sending a blank message.
      isReply: false,
    ));
    customSlot++;
  }

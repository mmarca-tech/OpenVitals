...
/// failed *read*, on the other hand, is still a failure `Result`: that is a
/// broken connection to Health Connect, not a verdict about the record.
class LoadBodyMeasurementForEditUseCase {
  ...
  Future<Result<BodyMeasurementEntry?>> call(
    BodyMeasurementType type,
    String recordId,
  ) async {
    final loaded =
        await _bodyRepository.loadBodyMeasurementEntry(type, recordId);
